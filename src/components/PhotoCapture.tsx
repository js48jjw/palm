...
// 반복적으로 리사이즈/압축하여 2.3MB 이하로 만드는 함수
async function resizeImageToMaxSize(file: File, maxSize = 2.3 * 1024 * 1024) {
  let quality = 0.8;
  let maxWidth = 1600;
  let maxHeight = 1600;
...
export const PhotoCapture: React.FC<PhotoCaptureProps> = ({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 2.3MB 초과 시 반복적으로 리사이즈 및 압축
    if (file.size > 2.3 * 1024 * 1024) {
      try {
        const blob = await resizeImageToMaxSize(file, 2.3 * 1024 * 1024);
        const resizedFile = new File([blob], file.name.replace(/\.[^.]+$/, '.jpg'), { type: 'image/jpeg' });
        console.log('최종 파일 크기:', resizedFile.size);
        if (resizedFile.size > 2.3 * 1024 * 1024) {
          setError('이미지 크기를 2.3MB 이하로 줄일 수 없습니다. 더 작은 이미지를 업로드해 주세요.');
          return;
        }
        // base64 인코딩 후 크기 체크
      ...
      if (file.size > 2.3 * 1024 * 1024) {
        setError('이미지 크기가 2.3MB를 초과합니다. 더 작은 이미지를 업로드해 주세요.');
        return;
      }
      // base64 인코딩 후 크기 체크
